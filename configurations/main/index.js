...
let config
let configFile = './' + (process.env.NODE_ENV ? process.env.NODE_ENV : 'development') + '.js'

try {
    config = require(configFile)
} catch (err) {
    if (err.code && err.code === 'MODULE_NOT_FOUND') {
        console.error('No config file matching NODE_ENV=' + process.env.NODE_ENV.toString() +
            '. Requires "' + __dirname.toString() + '/' + process.env.NODE_ENV.toString() + '.js"')
        process.exit(1)
    } else {
        throw err
    }
