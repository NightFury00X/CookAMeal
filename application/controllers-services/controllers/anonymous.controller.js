...
let Anonymous = {
    ...
    SignUp: async (req, res, next) => {
        try {
            ...
            if (registrationData.facebook && registrationData.facebook.facebookId) {
                ...
                if (checkFacebook) {
                    return ResponseHelpers.SetSuccessErrorResponse({message: 'Facebook already linked to another account.'}, res, CommonConfig.STATUS_CODE.CREATED)
                }
            }
            let result = await AnonymousService.SignUp(registrationData, req.files)
            if (!result) {
                return ResponseHelpers.SetSuccessErrorResponse('Unable to register user.', res, CommonConfig.STATUS_CODE.CREATED)
            }
            result.type = true
    ...
    ResetPassword: async (req, res, next) => {
        try {
            ...
            if (!email) {
                return ResponseHelpers.SetSuccessResponse({message: 'Missing email address.'}, res, CommonConfig.STATUS_CODE.OK)
            }
